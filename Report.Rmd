...
The website variable is therefore likely a strong predictor of our outcome variable
...
Using distance cut into categories for ease of use in our model. We group together values between 0-4000 and 4000+ km to make things even easier for our classifier.
...
barplot(counts, main="Distance Category Booking Distribution",
	    	xlab="Distance", col=c("cornsilk","lightblue"),
 	    	legend = rownames(counts))
...
We can confirm from this that SMS is not significant in our model, we can therefore remove it.
...
Need to be just a little bit careful because the sklearn label encoder says 0 is baggage and 1 is no baggage, the opposite of what we define initially but that's ok
